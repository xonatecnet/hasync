...
const App: React.FC = () => {
  ...
  // Connect WebSocket when authenticated with proper token
  useEffect(() => {
    if (isAuthenticated) {
      ...
      if (accessToken) {
        console.log('Setting WebSocket token and connecting...');
        wsClient.setAuthToken(accessToken);

        // Small delay to ensure token is set before connection
